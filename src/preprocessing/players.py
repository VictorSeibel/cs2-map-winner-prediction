...
def melt_player_stats(original_df):
    ...
    long_df[["team_player", "stats_var"]] = long_df["variable"].str.rsplit(
        "_", n=1, expand=True
    )
...
def hist_player_avg(long_df):
    ...
    long_df = long_df.sort_values(
        ["datetime", "game_id", "team_player"], ascending=[True, True, True]
    ...
    long_df[hist_stats_columns] = long_df.groupby("id")[stats_list].transform(
...
def pivot_player_stats(long_df):
    ...
    reshaping_columns_names = [
        f"{team_player}_{stat}" for stat, team_player in long_df.columns
    ]
...
def merge_player_stats(original_df, wide_stats_df):
    ...
    original_df = pd.merge(
        original_df, wide_stats_df, on="game_id", how="left", suffixes=("", "_right")
    )
